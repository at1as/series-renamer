...
func splitString(s, pwd string) {
	...
	if title != "" && episode != "" && season != "" && extension[0] != "" {
		// return the current path and the new path, mapped
		old_path := pwd + "/" + s
		new_path := pwd + "/" + title + "S" + season + "E" + episode + "." + extension[0]
		string_compare[old_path] = new_path
	} else {
		string_compare[pwd + "/" + s] = "Parse Error. Skipping"
	}
...
func main() {
	...
	if len(os.Args) >= 2 && (os.Args[1] == "-h" || os.Args[1] == "--help") {
		fmt.Println("Renamer will parse episodes of tv series and rename them with a clear, consistent style. \n\n" +
					"Example: \n\t a.series.title.s01e05.branding{year}.x264.HDTV.mp4  =>  A Series Title S01E05.mp4 \n" +
					"Usage: \n\t ./renamer /absolute/path/to/directory \n" +
					"Limitations: \n\t - won't work if s01e01 is [for some reason] dot-seperated \n\t - built and tested on *nix environment" +
